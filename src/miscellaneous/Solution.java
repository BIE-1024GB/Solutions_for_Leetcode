...
public class Solution {
    ...
    public char findKthBit(int n, int k) {
        ...
        if (k == mid) {
            ...
            char rev = findKthBit(n-1, len+1-k);
            return (rev=='1' ? '0' : '1');
        }
    }
    ...
    static class Fancy {
        ...
        public void append(int val) {
            long adjusted = ((val - add + mod) % mod) * modInverse(mul) % mod;
            list.add(adjusted);
        }
    ...
    public int largestSubmatrix(int[][] matrix) {
        ...
        for (int i = 0; i <= rows-1; i++) {
            ...
            int[] cr = new int[cols];
            System.arraycopy(matrix[i], 0, cr, 0, cols);
            Arrays.sort(cr);
    ...
    private boolean canRemoveFromRowSection(int[][] grid, int startRow, int endRow, int n, long value, int countInSection) {
    ...
    private boolean canRemoveFromColSection(int[][] grid, int startCol, int endCol, int m, long value, int countInSection) {
